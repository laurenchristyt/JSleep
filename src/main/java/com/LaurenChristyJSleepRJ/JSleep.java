...
import com.LaurenChristyJSleepRJ.Type;
...
public class JSleep {
   ...
   /*//  TEST MEMBUAT VOUCHER
   public static void makeVoucher() {
        try {
            String filePath = "src/json/voucher.json";
            JsonTable<Voucher>tableAccount = new JsonTable<>(Voucher.class, filePath);

            for (int i = 0;i<5;i++) {
                Type type = i% 2==0 ? Type.DISCOUNT : Type.REBATE;
                double cut = i % 2==0 ? 7500 : 10000;
                double min = i % 3==0 ? 150000f : 75000f;
                boolean used = i %2==0;
                tableAccount.add(new Voucher("Voucher" + i, i + 100,type,used,min,cut));
